...
class LinkIterator(val fileLocation:String, val startingIndex:Int) extends Iterator[String]{
  ...
  /**
   * Used to get the NYCREST object after next was called so is always index 1 prior
   * @return
   */
  def getInput(): NYCRest ={
    return restList(currentIndex - 1)
  }
